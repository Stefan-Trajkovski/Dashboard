...
export default function AddCategory() {
  ...
  const { editName, setShowAddCategory } = context
  return (
    <div className={` h-screen overflow-scroll

bg-[#F8F9FC] w-full sm:w-11/12 md:w-9/12 xl:w-6/12 font-mulish  opacity 
     min-h-screen sm:min-h-0 sm:h-auto sm:max-h-4/5  flex-col justify-center top-0 left-1/2 
     transform -translate-x-1/2 sm:top-auto  sm:rounded-lg md:top-1/2 md:-translate-y-1/2 absolute z-50 
     p-4 sm:p-6 md:p-8 lg:p-10   

  `}>

      <div className={` flex flex-col-reverse sm:flex-row sm:justify-between w-full `}>
        <p className='text-md font-bold w-12/12 md:w-6/12 xl:w-2/5 flex justify-center  tracking-tighter'>Add New Category</p>
        <span onClick={() => { setShowAddCategory(false) }} className=' cursor-pointer text-lg w-12/12  xl:w-8/12  flex justify-end items-center pe-1'><IoMdClose /></span>
      </div>
      ...
      <div className=' w-full   flex flex-col  sm:flex-row '>
        ...
        <div className='xl:w-8/12  lg:w-8/12   md:w-7/12    space-y-5 bg-[#F8F9FC] flex-col mt-5'  >
          ...
          <div className=' rounded p-6 pt-3 bg-white'>
            <p className='py-2 text-[#B58C43] font-bold text-[18px]'>Add Image</p>
          ...
          <div className=' rounded p-6 pt-3 bg-white'>
            <p className='py-2 text-[#B58C43] font-bold text-[18px]'>Title</p>

            <div className='flex justify-between w-full space-x-2 md:space-x-4'>
              <input className='px-4 rounded bg-[#F8F9FC] w-[100%] p-3 ' placeholder='Write the title' />
